...
public class AnnouncementService {
  ...
  public AnnouncementPostResponse addAnnouncement(AnnouncementPostRequest request) {
    ...
    List<AnnouncementContents> contents =
        request.getContents().stream()
            .map(trRequest -> AnnouncementContents.builder()
                .announcement(announcement)
  ...
  private void filterContentByLanguage(Announcement announcement, String languageCode) {
    ...
    AnnouncementContents translatedContent = announcement.getContents().stream()
  ...
  public AnnouncementEditResponse editAnnouncement(Long announcementId, AnnouncementEditRequest request) {
    ...
    Announcement announcement = announcementRepository.findById(announcementId)
        .orElseThrow(() -> new NotFoundException("Announcement", String.valueOf(announcementId), "공지사항을 찾을 수 없습니다."));
    ...
    if (request.getContents() != null && !request.getContents().isEmpty()) {
      ...
      // 새로운 콘텐츠 추가
      List<AnnouncementContents> updatedContents = request.getContents().stream()
          .map(trRequest -> AnnouncementContents.builder()
              .announcement(announcement)
              .languageCode(trRequest.getLanguageCode())
              .title(trRequest.getTitle())
    ...
    List<AnnouncementTrResponse> contentResponses = updatedAnnouncement.getContents().stream()
        .map(content -> AnnouncementTrResponse.builder()
